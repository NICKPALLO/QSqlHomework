    ...
    , ui(new Ui::MainWindow)
{
    ...
    dataDb = new Widget(this);
    dataBase = new DataBase(this);
    msg = new QMessageBox(this);
    dataForConnect.resize(NUM_DATA_FOR_CONNECT_TO_DB);
    dataBase->AddDataBase(POSTGRE_DRIVER, DB_NAME);

    connect(dataDb, &Widget::sig_sendData, this, [&](QVector<QString> receivData){
...
{
    ///Если мы не получали данных то TableView нужно удалить
    /// Если получали, то ui->TableWid->layout() удалит все сам
    if(tbView == nullptr)
    {
        dataBase->deleteTableView();
    }
...
{
    if(ui->cb_category->currentIndex( ) + 1 == requestAllFilms)
    {
        auto getAllTab = [this]{dataBase->getAllTable();};
        QFuture<void> futRcvData = QtConcurrent::run(getAllTab);
    }
    else if(ui->cb_category->currentIndex( ) + 1 == requestComedy)
    {
        auto reqToDb = [this]{dataBase->RequestToDB(comedyRequest);};
        QFuture<void> futRcvData = QtConcurrent::run(reqToDb);
    }
    else
    {
        auto reqToDb = [this]{dataBase->RequestToDB(horrorRequest);};
        QFuture<void> futRcvData = QtConcurrent::run(reqToDb);
    }
}
